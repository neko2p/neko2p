...
impl DNSPack {
    ...
    fn parse_packet(mut bytes: &[u8]) -> Self {
...
impl DNSResolver {
    pub fn new(nameserver: &str) -> Self {
        Self {
    ...
    pub async fn query(&self, domain: &str) -> IOResult<IpAddr> {
        ...
        sock.send_to(&pack.build_packet(), &self.nameserver).await?;
        ...
        P: ProxyConnection + Unpin,
    {
        ...
        conn.send(
            &pack.build_packet(),
            Network::Udp((Addr::from_str(&self.nameserver).unwrap(), 53)),
        )
        .await?;
        let mut buf = vec![0; UDP_MAX_PACK_SIZE];
